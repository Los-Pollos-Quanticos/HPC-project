    ...
    for (int i = 0; i < NP; i++)
    {
        ...
        for (int i = 0; i < NP; i++)
        {
            if (population[i].state == 0)
            {
                for (int j = 0; j < NP; j++)
                {
                    if (population[j].state == 2 && population[i].incubation_days != INCUBATION_DAYS)
                    { // Discard the NEWLY INFECTED!
                        // Calculate distance
